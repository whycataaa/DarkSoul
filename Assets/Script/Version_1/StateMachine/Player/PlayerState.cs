...
//        Debug.Log("Enter: "+GetType().ToString());
    ...
    //处理受击
    public virtual void HandleAttackReceived(AttackInfo info)
    {

        SetZeroVelocity();
        playerController.rb.AddForce(-playerController.player.transform.forward*playerController.hitForce,ForceMode.Impulse);

        switch(info.weaponType)
        {
            case WeaponType.Light:
                playerStateMachine.ChangeState(typeof(PlayerLightHitState));
                break;
            case WeaponType.Heavy:
                playerStateMachine.ChangeState(typeof(PlayerHeavyHitState));
                break;
